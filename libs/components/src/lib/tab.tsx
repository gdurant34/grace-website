...
const active = 'bg-amber-800 bg-opacity-5 shadow-sm text-cyan-700 px-4 py-3 flex items-center space-x-4 rounded-md font-bold';
const inactive = 'relative px-4 py-3 flex items-center space-x-4 text-gray-600 group rounded-md hover:bg-teal-500 hover:bg-opacity-10';
      ...
      <Root defaultValue="tab1" onValueChange={setCurrentValue}>
        <div className="p-6 flex justify-center">
          <div className="z-20 relative text-cyan-900 bg-stone-100 bg-opacity-50 rounded-sm text-lg sm:max-w-screen-md md:max-w-screen-lg lg:max-w-screen-xl">
            <List className="p-8 flex justify-center" aria-label="Manage your account">
              {tabs.map((tab) => (
                <Trigger
                  key={tab.id}
                  className={`px-4 ${tab.value === currentValue ? active : inactive}`}
                  value={tab.value}
                >
                  {tab.title}
                </Trigger>
                ...
                <div className="w-full text-center">
                  ...
                  <div className="m-4 flex justify-center space-x-4">
                    {tab.github.map((link) => (
                      ...
                      >
                        <a href={link.link} className="text-cyan-900 text-opacity-90">
                          {link.description}
                        </a>
                      </li>
                    ))}
